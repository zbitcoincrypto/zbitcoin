...
#if defined(NDEBUG)
# error "Zbitcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6756ec404b4e8b2a59455ecc58e9c0b6c941864ce17557902d97");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zbitcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Zbitcoin Signed Message:\n";
...
{
    // Zbitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Zbitcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Zbitcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Zbitcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zbitcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zbitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd31e1a9830722735e6f4eab5b96166fea6ee49c97406d0dfd7259f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is ZbitCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045ea6fcb8aa124aedcda59989148be4e85e6f151811e774b65c1e127f9879b3d2e425cdf6b94cf552a16095ab52ccf2c2d51a5f836fbdedfbed514bbecd1218c2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1526821023;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084642761;

        if (fTestNet)
        {
            block.nTime    = 1526820989;
            block.nNonce   = 386758058;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7b0d72ec65a1f3b6ab2d1029c6e2a4f0258030484b6bd22d95256bbc0cb02d37"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xbc, 0xd2 }; // Zbitcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZbitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZbitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZbitcoinMiner(CWallet *pwallet)
{
    printf("ZbitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zbitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ZbitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZbitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZbitcoinMiner, pwallet));
}
